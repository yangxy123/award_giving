...
public class AwardGivingServiceImpl implements AwardGivingService {
	...
	public void notice(NoticeReq noticeReq) {
		...
		List<BetInfoEntity> list = betInfoMapper
				.selectList(new QueryWrapper<BetInfoEntity>().lambda().eq(BetInfoEntity::getIssue, noticeReq.getIssue())
				...
				for (String key : countMap.keySet()) {
					...
					if (key.length() == 2) {
						List<BetInfoEntity> winList = betList.stream()
								.filter(vo -> vo.getCode().indexOf(key + ",") >= 0 || vo.getCode().endsWith(key) && vo.getMethodId()=="2d包组玩法")
								.collect(Collectors.toList());
						winList.forEach(vo -> {
						...
						List<BetInfoEntity> winList = betList.stream()
								.filter(vo -> vo.getCode().indexOf(key + ",") >= 0 || vo.getCode().endsWith(key)
										|| (vo.getCode().indexOf(key.substring(key.length() - 2, key.length()) + ",") >= 0  && vo.getMethodId()=="2d包组玩法")
										|| (vo.getCode().endsWith(key.substring(key.length() - 2, key.length()))  && vo.getMethodId()=="3d包组玩法"))
								.collect(Collectors.toList());
						winList.forEach(vo -> {
							vo.setBonus(vo.getBonus() * multiple);
						});
						...
						List<BetInfoEntity> winList = betList.stream()
								.filter(vo -> (vo.getCode().indexOf(key.substring(key.length() - 2, key.length()) + ",") >= 0  && vo.getMethodId()=="2d包组玩法")
										|| (vo.getCode().endsWith(key.substring(key.length() - 2, key.length())) && vo.getMethodId()=="2d包组玩法")
										|| (vo.getCode().indexOf(key.substring(key.length() - 3, key.length()) + ",") >= 0 && vo.getMethodId()=="3d包组玩法")
										|| (vo.getCode().endsWith(key.substring(key.length() - 3, key.length())) && vo.getMethodId()=="3d包组玩法")
										|| vo.getCode().indexOf(key.substring(key.length() - 4, key.length()) + ",") >= 0
			...
			try {
				...
				for (String key : group7_2d.keySet()) {
					...
					if (key.length() == 2) {
						...
						List<BetInfoEntity> winList = betList.stream()
								.filter(vo -> vo.getCode().indexOf(key.substring(key.length() - 2, key.length()) + ",") >= 0
			...
			try {
				...
				for (String key : group7_3d.keySet()) {
					...
					if (key.length() == 3) {
						...
						List<BetInfoEntity> winList = betList.stream()
								.filter(vo -> vo.getCode().indexOf(key.substring(key.length() - 3, key.length()) + ",") >= 0
		...
		new Thread(() -> {// pl2
			try {
				...
				List<BetInfoEntity> winList = betList.stream()
						...
						&& (noticeReq.getCode().indexOf(vo.getCode().split(",")[0] + ",") < noticeReq.getCode().indexOf(vo.getCode().split(",")[1] + ",")
		...
		new Thread(() -> {// pl3
			try {
				...
				List<BetInfoEntity> betList = list.stream().filter(vo -> vo.getMethodId() == 7)
						.collect(Collectors.toList());
				List<BetInfoEntity> winList = betList.stream()
						.filter(vo -> noticeReq.getCode().indexOf(vo.getCode().split(",")[0] + ",") >= 0 && noticeReq.getCode().indexOf(vo.getCode().split(",")[1] + ",") >= 0
						&& (noticeReq.getCode().indexOf(vo.getCode().split(",")[0] + ",") < noticeReq.getCode().indexOf(vo.getCode().split(",")[1] + ",") && (noticeReq.getCode().indexOf(vo.getCode().split(",")[1] + ",") < noticeReq.getCode().indexOf(vo.getCode().split(",")[2] + ",")
		...
		new Thread(()->{//2d头、尾、头尾
			try {
				...
				List<BetInfoEntity> winList = betList.stream().filter(vo->
					((vo.getCode().indexOf(headCode+",") >= 0 || vo.getCode().endsWith(headCode)) && (vo.getMethodId() == "2d头玩法" || vo.getMethodId() == "2d头尾玩法"))
					|| ((vo.getCode().indexOf(endCode+",") >= 0 || vo.getCode().endsWith(endCode)) && (vo.getMethodId() == "2d尾玩法" || vo.getMethodId() == "2d头尾玩法"))
				).collect(Collectors.toList());
				allWinList.addAll(winList);
			}catch (Exception e) {
				// TODO: handle exception
		...
		new Thread(()->{//3d头、尾、头尾
			try {
				...
				List<BetInfoEntity> winList = betList.stream().filter(vo->
					((vo.getCode().indexOf(headCode+",") >= 0 || vo.getCode().endsWith(headCode)) && (vo.getMethodId() == "3d头玩法" || vo.getMethodId() == "3d头尾玩法"))
					|| ((vo.getCode().indexOf(endCode+",") >= 0 || vo.getCode().endsWith(endCode)) && (vo.getMethodId() == "3d尾玩法" || vo.getMethodId() == "3d头尾玩法"))
				).collect(Collectors.toList());
				allWinList.addAll(winList);
			}catch (Exception e) {
				// TODO: handle exception
		...
		new Thread(()->{//4D尾玩法
			try {
				...
				List<BetInfoEntity> betList = list.stream().filter(vo -> vo.getMethodId() == 7)
						.collect(Collectors.toList());
				String endCode = codeList.get(17).substring(3,6);
				List<BetInfoEntity> winList = betList.stream().filter(vo->vo.getCode().indexOf(endCode+",") >= 0 || vo.getCode().endsWith(endCode)
				).collect(Collectors.toList());
				allWinList.addAll(winList);
			}catch (Exception e) {
				// TODO: handle exception
		...
		//未中奖订单ID
		List<BetInfoEntity> notWinList = list.stream().filter(vo->!winIdList.contains(vo.getProjectId())).collect(Collectors.toList());
	}
